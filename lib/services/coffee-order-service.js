...
module.exports = class OrderService {
    ...
    static async createOrder({name, address, coffeeType, quantity, subscription}) {
        ...
        const sendEmail = await sendAWSEmail(coffeeType, quantity)
    ...
    static async getOneOrder(id) {
        const singleOrder = await CoffeeOrder.selectSingleOrder(id)
        
        return {details: `order details for order_id:${id}`, data: singleOrder}
    }
    ...
    static async deleteOneOrder(id) {
        const deletedOrder = await CoffeeOrder.deleteSingleOrder(id)

        return {details: `deleted order for order_id:${id}`, data: deletedOrder}
    }
